...
export const Camera = new class extends OrthographicCamera implements IUpdatable {
    ...
    constructor(fructumSize = DEFAULT_FRUCTUM) {
        ...
        super(fructumSize * aspect / -2, fructumSize * aspect / 2, fructumSize / 2, fructumSize / -2, 0.1, 100);
    ...
    public update(): void {
        if (this._followObject) {
            this.position.set(this._followObject.position.x, this._followObject.position.y, 1);
        }
    }
    ...
    public TransformToWorldCoordinate(vector: Vector3): Vector3 {
        const newVector: Vector3 = new Vector3((vector.x / window.innerWidth) * 2 - 1, - (vector.y / window.innerHeight) * 2 + 1, 0.5);
